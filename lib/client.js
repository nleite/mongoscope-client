...
function Client(opts){
  ...
  this.io = socketio.connect(this.config.scope);
}
...
function getRouter(def){
  ...
  function params(route, fragment){
    ...
    debug('params', route, fragment, JSON.stringify(p));
    if(!p[0]){
      return [];
    }
...
function Subscription(client, url, handler){
  ...
  process.nextTick(function(){
    this.client.io.addListener(this.url, this.handler).emit(url, this.payload);
  }.bind(this));
}

Subscription.prototype.unsubscribe = function(){
  this.client.io.removeAllListeners(this.url)
    .emit(this.url + '/unsubscribe', this.payload);
};

Client.prototype.subscribe = function(url, handler){
  assert(this.io.connected, 'socketio not connected');
